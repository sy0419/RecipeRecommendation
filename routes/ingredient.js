...
const { Ingredient } = require('../models');  // Ingredient 모델 불러오기 Import Ingredient model
...
router.post('/', async (req, res) => {
    try {
        ...
        if (!name) {
            return res.status(400).json({ error: 'Name is required' });  // name 누락 시 400 에러 응답 Return 400 if name is missing
        }

        const newIngredient = await Ingredient.create({ name, category });  // DB에 새 재료 생성 Create new ingredient in DB
        res.status(201).json(newIngredient);  // 생성 완료 후 201 응답 Return 201 after creation
    } catch (err) {
