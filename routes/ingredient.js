const express = require('express');  // 익스프레스 모듈 불러오기 (Express module import)
const ingredient = require('../models/Ingredient');  // Ingredient 모델 불러오기 (Import Ingredient model)
const { json } = require('sequelize');  // Sequelize의 json 기능 불러오기 (Import json from Sequelize)

const router = express.Router();  // 익스프레스 라우터 생성 (Create Express router)

// GET /ingredient - 재료 목록 조회 API (API to get list of ingredients)
router.get('/ingredient', async (req, res) => {
    try {
        const ingredient = await ingredient.findAll();  // DB에서 모든 재료 조회 (Fetch all ingredients from DB)
        res.json(ingredient);  // JSON 형식으로 응답 (Respond with JSON)
    } catch (err) {
        res.status(500), json({ error: 'Failed to fetch ingredient' });  // 에러 시 500 응답 (Respond 500 on error)
    }
});

// POST /ingredient - 재료 추가 API (API to add a new ingredient)
